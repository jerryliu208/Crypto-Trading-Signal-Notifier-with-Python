...
def help_handler(message):
    help_msg = ""
    help_msg += "/subscribe 用此指令訂閱，當訊號出現時機器人就會通知你喔！\n"
    help_msg += "/signal 用此指令，可以主動查詢您輸入的交易對之訊號～"
    bot.reply_to(message, help_msg)

@bot.message_handler(commands=['start', 'subscribe'])
def start_handler(message):
    start_msg = "\n" + "使用 /help 指令來了解機器人使用方式吧！"
    #讀檔取得所有訂閱者
...
def get_symbol_signal(message):
    user_input_text_arr = message.text.split(",")
    # 確認使用者輸入格式正確
    if len(user_input_text_arr) != 2:
        bot.reply_to(message, "輸入格式錯誤！請參考/symbol指令的說明！")
        return
    # 取得使用者輸入的交易對和時間週期
    symbol, interval = user_input_text_arr[0], user_input_text_arr[1]
    # 查訊使用者input的訊號
    signal = None
    try:
        signal = SignalStrategy(symbol=symbol, interval=interval).signal_analyze_using_kd_macd()
    except Exception as e:
        bot.send_message(message.chat.id, "分析錯誤：" + str(e))
        return
